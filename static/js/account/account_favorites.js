...
export function initFavoritesPage() {
    ...
    // Use event delegation for edit nickname buttons
    document.addEventListener('click', (e) => {
        const editButton = e.target.closest('.edit-nickname-btn');
        if (editButton) {
            e.preventDefault();
            const card = editButton.closest('.favorite-card');
            const displayName = card?.querySelector('.display-name[data-favorite-id]');
            const favoriteId = displayName?.dataset.favoriteId;
            if (favoriteId) {
                toggleNicknameEdit(favoriteId);
            }
        }
    ...
    // Use event delegation for save nickname buttons
    document.addEventListener('click', (e) => {
        const saveButton = e.target.closest('.save-nickname-btn');
        if (saveButton) {
            e.preventDefault();
            const editContainer = saveButton.closest('[id^="nickname-edit-"]');
            const favoriteId = editContainer?.id.replace('nickname-edit-', '');
            if (favoriteId) {
                saveNickname(favoriteId);
            }
        }
    });

    // Use event delegation for unfavorite buttons
    document.addEventListener('click', (e) => {
        const deleteButton = e.target.closest('.delete-button');
        if (deleteButton && deleteButton.closest('.favorite-card')) {
            e.preventDefault();
            const locationId = deleteButton.dataset.locationId;
            if (locationId) {
                unfavoriteLocation(parseInt(locationId, 10));
            }
        }
    });
}
